...
public class XboxOneController extends AbstractXboxController {
        ...
        switch (buffer.get())
        {
            case 0x20:
                if (buffer.remaining() < 17) {
                    Log.e("Controller", "XBone button/axis read too small: " + buffer.remaining());
                    return false;
                }
            ...
            case 0x07:
                if (buffer.remaining() < 4) {
                    Log.e("Controller", "XBone mode read too small: " + buffer.remaining());
                    return false;
                }
    ...
    protected boolean doInit() {
        ...
        for (InitPacket pkt : INIT_PKTS) {
            ...
            if (res != data.length) {
                Log.w("Controller", "Initialization transfer failed: "+res);
                return false;
            }
        }
    ...
    private void sendRumblePacket() {
        ...
        if (res != data.length) {
            Log.w("Controller", "Rumble transfer failed: "+res);
        }
    }
