    ...
    private Thread createInputThread() {
        return new Thread() {
            public void run() {
                ...
                System.out.println("notify added");
                ...
                while (!isInterrupted() && !stopped) {
                    ...
                    do {
                        ...
                        if (res == -1 && SystemClock.uptimeMillis() - lastMillis < 1000) {
                            Log.w("Controller", "Detected device I/O error");
    ...
    public boolean start() {
        ...
        for (int i = 0; i < device.getInterfaceCount(); i++) {
            ...
            if (!connection.claimInterface(iface, true)) {
                Log.w("Controller", "Failed to claim interfaces");
                return false;
            }
        }
        ...
        for (int i = 0; i < iface.getEndpointCount(); i++) {
            ...
            if (endpt.getDirection() == UsbConstants.USB_DIR_IN) {
                if (inEndpt != null) {
                    Log.w("Controller", "Found duplicate IN endpoint");
                    return false;
                }
                inEndpt = endpt;
            }
                ...
                if (outEndpt != null) {
                    Log.w("Controller", "Found duplicate OUT endpoint");
                    return false;
                }
                outEndpt = endpt;
            }
        ...
        if (inEndpt == null || outEndpt == null) {
            Log.w("Controller", "Missing required endpoint");
            return false;
        }
